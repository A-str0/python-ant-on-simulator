...
class Colony:
    def __init__(self, colony_id: str, ant_count: int, food: int) -> None:
        if not isinstance(colony_id, str):
            raise TypeError(f"colony_id must be str, got {type(colony_id).__name__}")
        if not isinstance(ant_count, int) or isinstance(ant_count, bool):
            raise TypeError(f"ant_count must be int, got {type(ant_count).__name__}")
        if not isinstance(food, int) or isinstance(food, bool):
            raise TypeError(f"food must be int, got {type(food).__name__}")
        
        if ant_count < 0:
            raise ValueError(f"ant_count must be non-negative, got {ant_count}")
        if food < 0:
            raise ValueError(f"food must be non-negative, got {food}")
